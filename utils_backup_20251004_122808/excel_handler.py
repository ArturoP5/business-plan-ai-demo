...
def leer_excel_datos(uploaded_file):
    ...
    try:
        ...
        # Agregar los NUEVOS campos de descripción del negocio
        datos['info_general']['descripcion_actividad'] = str(info_dict.get('Descripción de la Actividad', ''))
        datos['info_general']['productos_servicios'] = str(info_dict.get('Productos/Servicios Principales', ''))
        datos['info_general']['cuota_mercado'] = float(info_dict.get('Cuota de Mercado (%)', 0) or 0)
        datos['info_general']['posicionamiento'] = str(info_dict.get('Posicionamiento de Precios', 'Medio'))
        datos['info_general']['ventajas_competitivas'] = str(info_dict.get('Ventajas Competitivas', ''))
        datos['info_general']['clientes_objetivo'] = str(info_dict.get('Clientes Objetivo', ''))
