...
class APIDataCollector:
    ...
    def _get_pib_ine(self) -> Optional[float]:
        """Obtiene el crecimiento del PIB del Banco de España"""
        try:
            # API del Banco de España - PIB tasa variación interanual
            url = "https://api.bde.es/bie/series/573901.json"
            ...
            if response.status_code == 200:
                ...
                if "series" in data and len(data["series"]) > 0:
                    ...
                    if "datos" in serie and len(serie["datos"]) > 0:
                        ...
                        ultimo_dato = serie["datos"][-1]
                        if "valor" in ultimo_dato:
                            return float(ultimo_dato["valor"])
        except Exception as e:
            print(f"Error obteniendo PIB del Banco de España: {e}")
            
        # Si falla, usar valor conocido actual
        return 2.8
    ...
    def _get_desempleo_ine(self) -> Optional[float]:
        ...
        try:
            # EPA - Tasa de paro total nacional
            url = f"{self.ine_base_url}DATOS_SERIE/EPA13937?nult=1"
        ...
        # Si falla, intentar con Banco de España
        try:
            url_bde = "https://api.bde.es/bie/series/270850.json"
            response = requests.get(url_bde, timeout=self.timeout)
            if response.status_code == 200:
                data = response.json()
                if "series" in data and len(data["series"]) > 0:
                    serie = data["series"][0]
                    if "datos" in serie and len(serie["datos"]) > 0:
                        return float(serie["datos"][-1]["valor"])
        except:
            pass
    ...
    def _get_euribor(self) -> Optional[float]:
        """Obtiene el Euribor a 12 meses actualizado"""
        try:
            # Banco de España - Euribor 12 meses
            url = "https://api.bde.es/bie/series/819613.json"
            response = requests.get(url, timeout=self.timeout)
            ...
            if response.status_code == 200:
                ...
                if "series" in data and len(data["series"]) > 0:
                    ...
                    if "datos" in serie and len(serie["datos"]) > 0:
                        ...
                        ultimo_valor = serie["datos"][-1]["valor"]
                        return float(ultimo_valor)
        except Exception as e:
            print(f"Error obteniendo Euribor del Banco de España: {e}")
            
        # Si falla, intentar con BCE
        try:
