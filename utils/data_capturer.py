...
def capturar_datos_completos() -> Dict[str, Any]:
    ...
    # 1. DATOS DE EMPRESA (con las keys correctas del sidebar)
    datos['info_empresa'] = {
        ...
        'productos_servicios': st.session_state.get('productos_servicios_sidebar', 'N/A'),
        ...
        'cuota_mercado': st.session_state.get('cuota_mercado_sidebar', 'N/A'),
        'clientes_objetivo': st.session_state.get('clientes_objetivo_sidebar', 'N/A'),
    }
...
def guardar_datos_demo(datos_demo: Dict[str, Any]) -> None:
    ...
    MAPEO DE NOMBRES:
    - ventaja_competitiva_principal (demo) → ventaja_competitiva_clave (key)
    """
    if 'info_general' in datos_demo:
        ...
        st.session_state['descripcion_actividad_sidebar'] = info.get('descripcion_actividad', '')
        st.session_state['productos_servicios_sidebar'] = info.get('productos_servicios', '')
        ...
        # MAPEO: ventaja_competitiva_principal → ventaja_competitiva_clave
        st.session_state['ventaja_competitiva_clave'] = info.get('ventaja_competitiva_principal', '')
        st.session_state['principales_riesgos'] = info.get('principales_riesgos', '')
        st.session_state['cuota_mercado_sidebar'] = info.get('cuota_mercado', '')
        st.session_state['clientes_objetivo_sidebar'] = info.get('clientes_objetivo', '')
